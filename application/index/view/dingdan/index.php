            ...
            <form method="get" action="{:url('index')}" id="searchForm">
                <div class="col-xs-8">
                    ...
                    <select name="st" id="" class="form-control">
                        ...
                        <?php foreach(app\common\model\Dingdan::$arrStatus as $k=>$v){?>
                            <option value="{$k}" <?php echo isset($_GET['st'])?$k===(int)$_GET['st']?'selected':'':'';?>>{$v}</option>
                        <?php }?>
                    </select>
