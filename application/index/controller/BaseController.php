...
class BaseController extends Controller {
		...
		public function __construct() {
			...
			if(!session('admin_edu') && !in_array($current_request,$not_logins)){
        ...
        protected function dealImg($file,$type){
            $upload_dir = ROOT_PATH . 'public/uploads/'.$type.'/' ;
            $info = $file->move($upload_dir);
            if($info){
                // 输出 20160820/42a79759f284b767dfcb2a0197904287.jpg
                $url_path = "/uploads/$type/".$info->getSaveName();
            }else{
                // 上传失败获取错误信息
        ...
        protected function thumbImg($type,$imgInfo,$width,$height){
         ...
         //  dump($path);exit;
          ...
          //  $image->crop($width,$height)->save($save_thumb_img);
            ...
            $save_thumb_img_url = '/uploads/'.$type.'/'.explode('.',$imgInfo['info']->getSaveName())[0].'_mobile.'.$deliter[1];
